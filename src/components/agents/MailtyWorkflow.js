    ...
    <>
      <Header
        ...
        colors={isLight ? {
          ...
          bg: 'bg-gradient-to-r from-violet-50/90 via-fuchsia-100/80 to-transparent',
          button: 'bg-violet-600/80'
        } : {
          border: 'border-violet-500/30',
          textColor: 'text-white',
            bg: 'bg-gradient-to-r from-violet-950/70 via-fuchsia-900/60 to-transparent',
          button: 'bg-violet-600/70'
        }}
        messages={[]} clearHistory={()=>{}}
      />
      <main className="pt-28 px-4 pb-12 max-w-7xl mx-auto font-sans">
      ...
      {connected && (
        <div className="flex flex-col gap-4 mt-2">
          {/* Toolbar */}
          <div className="rounded-xl border bg-white/80 dark:bg-white/5 backdrop-blur-md px-4 py-3 flex flex-col md:flex-row gap-3 md:items-center md:justify-between shadow-sm dark:shadow-[0_0_0_1px_rgba(255,255,255,0.05)] sticky top-24 z-30 lg:static">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className={`rounded-2xl border bg-white/80 dark:bg-white/5 backdrop-blur-xl flex flex-col overflow-hidden shadow-sm dark:shadow-[0_0_0_1px_rgba(255,255,255,0.06)] ${detail? 'hidden lg:flex' : 'flex'}`}> 
              ...
              <div className="flex-1 overflow-y-auto text-sm divide-y dark:divide-white/5 custom-scroll">
                ...
                {emails.map(m=> (
                  <button
                    ...
                    className={`group w-full text-left px-4 py-3 flex flex-col gap-1 transition relative overflow-hidden ${selected===m.id? 'bg-gradient-to-r from-violet-500/15 to-fuchsia-500/15 dark:from-violet-500/20 dark:to-fuchsia-500/25' : 'hover:bg-violet-500/10 dark:hover:bg-violet-500/15'}`}
                  >
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-violet-500/15 text-violet-700 dark:text-violet-300 font-medium tracking-wide">
                        {m.fromName || 'Exp.'}
                      </span>
                      <span className="font-medium truncate flex-1 text-[13px] text-gray-700 dark:text-gray-200 group-hover:text-violet-700 dark:group-hover:text-violet-200">
                        {m.subject||'(Sans objet)'}
                      </span>
                    </div>
                    <p className="text-[11px] line-clamp-2 text-gray-500 dark:text-gray-400">{m.snippet}</p>
                    <span className="absolute inset-y-0 left-0 w-0.5 bg-gradient-to-b from-violet-500 to-fuchsia-500 opacity-0 group-hover:opacity-60 transition"/>
                  </button>
                ))}
              </div>
            ...
            <div className={`rounded-2xl border bg-white/80 dark:bg-white/5 backdrop-blur-xl flex flex-col overflow-hidden lg:col-span-2 shadow-sm dark:shadow-[0_0_0_1px_rgba(255,255,255,0.06)] ${detail? 'block' : 'block lg:block'} ${!detail? 'hidden lg:flex' : ''}`}>
              ...
              {detail && (
                <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scroll">
                  ...
                  <div className="rounded-lg border border-violet-200 dark:border-violet-500/20 bg-white/70 dark:bg-white/5 p-4 text-[13px] leading-relaxed whitespace-pre-wrap shadow-inner max-h-72 overflow-y-auto custom-scroll sm:max-h-80">
                    {detail.body || detail.snippet}
                  </div>
      {aiReply && (
                    <div className="rounded-lg border border-fuchsia-300/40 dark:border-fuchsia-500/30 bg-gradient-to-br from-fuchsia-50/80 to-violet-50/70 dark:from-fuchsia-900/20 dark:to-violet-900/10 p-4 text-[12px] space-y-3">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <label className="text-[10px] font-semibold text-fuchsia-600 dark:text-fuchsia-300">Ton</label>
                          <select
                            value={tone}
                            onChange={e=>setTone(e.target.value)}
                            className="h-7 px-2 rounded-md text-[11px] bg-white/80 dark:bg-white/10 border border-fuchsia-300/50 dark:border-fuchsia-500/30 focus:outline-none focus:ring-2 focus:ring-fuchsia-400/40"
                          >
        ...
        {aiAutoTriggered && <span className="text-[10px] px-2 py-0.5 rounded-full bg-fuchsia-500/15 text-fuchsia-600 dark:text-fuchsia-300 border border-fuchsia-400/30 animate-pulse">Auto</span>}
                      ...
                      <p className="whitespace-pre-wrap text-gray-700 dark:text-gray-200">{aiReply}</p>
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      rows={6}
                      className="w-full rounded-lg border bg-white/70 dark:bg-white/5 border-violet-200 dark:border-violet-500/30 focus:outline-none focus:ring-2 focus:ring-violet-400/50 text-sm p-3 resize-none custom-scroll"
      ...
      {error && <div className="mt-6 text-sm text-rose-600 dark:text-rose-400">{error}</div>}
      </main>
    </>
  );
}
