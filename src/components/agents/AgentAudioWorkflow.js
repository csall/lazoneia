...
export default function AgentAudioWorkflow({
	...
	useEffect(() => {
		if (transcriptionCompleted) {
			console.log('Transcription completed, attempting to click the submit button.');
			...
			<div className="container mx-auto px-4 py-4">
				<div className="flex flex-col md:flex-row gap-4 items-center mb-6">
					<motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="w-24 h-24 md:w-32 md:h-32">
						<Image src={branding?.botImage || botImage} alt={branding?.name} width={128} height={128} className="w-full h-full drop-shadow-[0_0_20px_rgba(139,92,246,0.5)]" priority />
					</motion.div>
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="text-center md:text-left">
						<h1 className="text-3xl md:text-4xl font-extrabold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-white to-white drop-shadow-lg">{branding?.name}</h1>
						<p className={`text-sm md:text-base ${colors.textColor} max-w-xl drop-shadow-md`}>{branding?.description}</p>
					</motion.div>
				</div>
						...
						<form onSubmit={handleSubmit} className="space-y-3">
							<div className="relative flex items-center">
												...
												<div className="relative">
													...
													{micState === 'recording' && (
														<div className="absolute inset-0 flex items-center justify-center">
															<ChatGPTMicAnimation text="Recording..." amplitude={micAmplitude} color={`bg-gradient-to-r ${colors.gradientFrom} ${colors.gradientTo}`} />
														</div>
													)}
													...
													{showMic && (
														<motion.button
															...
															className={`absolute top-2 right-2 bg-gradient-to-br ${colors.buttonGradientFrom} ${colors.buttonGradientTo} ${colors.buttonHoverFrom} ${colors.buttonHoverTo} text-white rounded-full p-2 shadow-lg transition-all duration-200 select-none touch-none border-2 border-${colors.buttonGradientFrom.replace('from-', '')}/60 ${micState === 'recording' ? `scale-125 ring-4 ${colors.ringColor} ${colors.shadowColor} opacity-80` : ''} ${micState === 'transcribing' ? 'opacity-60 cursor-wait' : ''}`}
															aria-label={micState === 'idle' ? 'Démarrer l\'enregistrement' : micState === 'recording' ? 'Valider' : 'Transcription en cours'}
															style={{ touchAction: 'none', WebkitUserSelect: 'none', userSelect: 'none', WebkitTouchCallout: 'none', background: micState === 'recording' ? `linear-gradient(90deg, ${colors.buttonGradientFrom.replace('from-', '')} 60%, ${colors.buttonGradientTo.replace('to-', '')} 100%)` : undefined, opacity: micState === 'transcribing' ? 0.6 : 1, cursor: micState === 'transcribing' ? 'wait' : 'pointer' }}
															disabled={micState === 'transcribing'}
														>
															{micState === 'recording' ? (
																<svg className="h-7 w-7 opacity-80 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
								...
								<div className="flex gap-2 mt-4">
									...
									>
										{isLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : sendButtonLabel}
									</button>
									<button
										type="button"
										onClick={handleClear}
					...
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }} className={`bg-gradient-to-br ${colors.gradientFrom}/50 ${colors.gradientTo}/50 backdrop-blur-md p-4 rounded-xl shadow-lg border ${colors.borderColor} min-h-[240px] flex flex-col`}>
						...
						{isLoading ? (
							<div className="flex-1 flex flex-col items-center justify-center">
								...
								<p className={`mt-2 ${colors.textColor} text-sm`}>Préparation de la réponse...</p>
							</div>
						) : response ? (
							<div 
  ref={responseRef} 
...
>
							...
							<div className={`flex-1 flex flex-col items-center justify-center ${colors.textColor}/70`}>
