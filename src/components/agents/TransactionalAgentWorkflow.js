...
export default function TransactionalAgentWorkflow({ agent }) {
  ...
  const [targetLang, setTargetLang] = usePersistentState(
    "txn_target_lang",
    "français"
  );
  const handleLanguageChange = (e) =>
    setTargetLang(e.target?.value || targetLang);
  ...
  const languagesSafe = useMemo(
    () => languages.map((l, i) => ({ ...l, _kid: `lang-${i}-${l.value}` })),
    [languages]
  );
  ...
  useEffect(() => {
    if (firstLangChange.current) {
      firstLangChange.current = false;
      return;
    }
  ...
  useEffect(() => {
    if (firstToneChange.current) {
      firstToneChange.current = false;
      return;
    }
    const trimmed = sourceRef.current.trim();
    if (!trimmed) {
    ...
    if (!loading && result) {
      const raf = requestAnimationFrame(() => {
        ...
        if (area) {
          ...
          const need =
            area.scrollHeight > area.clientHeight + 4 &&
            area.scrollTop + area.clientHeight < area.scrollHeight - 4;
    ...
    >
      ...
      <div className="mx-auto w-full max-w-6xl px-4 pb-24 pt-8 md:pt-10 flex flex-col gap-8 transition-all">
        ...
        <div className="grid gap-8 md:grid-cols-2 items-start">
          ...
          <div
            ref={sourcePanelRef}
            className={`relative flex flex-col rounded-xl border backdrop-blur-sm ${
              isLight
          ...
          >
            ...
            <textarea
              className={`flex-1 w-full resize-none rounded-md border bg-transparent p-3 text-sm leading-relaxed focus:outline-none focus:ring-2 ${
                isLight
                  ? "border-gray-300 focus:ring-indigo-300"
                  : "border-white/15 focus:ring-fuchsia-500/40"
              }`}
            ...
            <div className="mt-2 flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400 select-none">
              ...
              {autoTriggered && (
                <span
                  className="ml-2 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-300 dark:border-indigo-400/40 animate-pulse"
                  title="Envoi automatique après changement de langue"
                >
                  Auto
                </span>
              )}
                ...
                className={`h-10 rounded-md text-sm font-medium transition flex items-center gap-2 px-4
                  ${
                    isLight
                      ...
                      : canSubmit
                      ? `bg-gradient-to-r ${
                          colors.buttonGradientFrom || "from-indigo-500"
                        } ${
                          colors.buttonGradientTo || "to-violet-600"
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs font-medium transition border text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800/60 active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:focus-visible:ring-offset-gray-900 ${
                        selected
                          ? "bg-gradient-to-r from-indigo-500/25 to-violet-500/25 text-indigo-700 dark:text-indigo-300 border-indigo-400/40 shadow-sm"
                          : "border-transparent"
                      }`}
                      role="option"
                    ...
                    >
                      <Image
                        ...
                        width={18}
                        height={18}
                        className="rounded object-cover ring-1 ring-black/5 dark:ring-white/10"
                        unoptimized
                      />
                      ...
                      {selected && (
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          className="h-3 w-3 ml-auto text-indigo-600 dark:text-indigo-300"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          strokeWidth="2"
              ...
              {loading && (
                <div className="absolute inset-0 flex flex-col gap-2">
                  {Array.from({ length: 10 }).map((_, i) => (
                    <div
                      key={i}
                      className="h-3 rounded bg-gray-300/60 dark:bg-white/10 animate-pulse"
              ...
              >
                ...
                {!loading && !error && !result && (
                  <p className="text-sm opacity-60 italic">
                    Le résultat apparaîtra ici après traitement…
                  </p>
                )}
              </div>
              {showBottomFade && (
